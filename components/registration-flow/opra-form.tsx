...
export default function OpraForm() {
    ...
    const handleFill = async () => {
        try {
            ...
            const res = await fetch('/api/pdf/fill-opra', {
                method: 'POST',
            ...
            if (res.status === 401) {
                toast.error('Unauthorized: ' + response.error);
                return;
            }
            if (!res.ok) {
            ...
            toast.success('PDF successfully generated and stored.\nLink: ' + response.pdfUrl)
        } catch (err) {
            console.error(err)
                    ...
                    <div className="grid gap-6">
                                    ...
                                    render={({field}) => (
                                        <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                            ...
                                            <div className="space-y-1 leading-none">
                                                <FormLabel>I confirm that I have read and accept the Cboe Customer
                                                    Agreement.</FormLabel>
                                                <FormMessage/>
                                            </div>
                                        </FormItem>
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Please scroll down to read the Nasdaq Customer
                                    Agreement.</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <ScrollArea className="h-[200px] w-full rounded-md border p-4">
                                    <div className="text-sm text-muted-foreground">
                                ...
                                <FormField
                                    ...
                                    render={({field}) => (
                                        <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                            ...
                                            <div className="space-y-1 leading-none">
                                                <FormLabel>I confirm that I have read and accept the Nasdaq Customer
                                                    Agreement.</FormLabel>
                                                <FormMessage/>
                                            </div>
                                        </FormItem>
