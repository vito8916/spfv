...
const AgreementsForm = () => {
  ...
  useEffect(() => {
    const loadTermsAndAcceptances = async () => {
      try {
        ...
        const tos = await getActiveTerms("terms_of_service");
        const privacy = await getActiveTerms("privacy_policy");

        allTerms.push(tos);
        allTerms.push(privacy);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const allAccepted = terms.every(
      (term) => formValues[`${term.type}Accepted`]
    );
    ...
    try {
      ...
      const termsToAccept = terms.filter(
        (term, index) => !existingAcceptances[index]
      );

      if (termsToAccept.length > 0) {
        await Promise.all(termsToAccept.map((term) => acceptTerms(term.id)));
      }

      // Update user agreements in database
      const { success: updateSuccess, error: updateError } = await updateUserAgreements(true, true);
      if (!updateSuccess) {
          ...
          {terms.map((term) => (
            <Card key={term.id}>
              <CardHeader>
                ...
                <CardDescription>Version {term.version}</CardDescription>
              </CardHeader>
              <CardContent>
        ...
        <div className="flex justify-end">
          <Button
            ...
            disabled={
              isLoading ||
              !terms.every((term) =>
                form.watch(`${term.type}Accepted` as keyof FormSchema)
              )
            }
          >
            {isLoading ? (
